...
export async function POST(request: NextRequest) {
  try {
    ...
    if (file) {
      ...
      // Parsear el archivo (CSV, Excel o PDF)
      const isPDF = file.name.endsWith(".pdf");
      const isExcel = file.name.endsWith(".xlsx") || file.name.endsWith(".xls");

      if (isPDF || isExcel) {
        movsBanco = await parsearExtracto(
          buffer,
          file.name,
    ...
    const { data: movsDB, error: movsError } = await supabase
      ...
      .select("id, fecha, descripcion, monto, tipo, conciliado, categoria:categorias_financieras!categoria_id(nombre)")
