  ...
  useEffect(() => {
    async function load() {
      ...
      const { data } = await supabase
        .from("productos")
        .select(
          "*, categorias_producto(id, nombre, slug), producto_imagenes(id, url, alt_text, orden, es_principal, focal_point), producto_variantes(id, nombre, sku, precio_override, stock_actual, atributos, activo), producto_proveedores(id, proveedor_id, costo, codigo_proveedor, es_principal)"
        )
        .eq("id", parseInt(id))
        .single();
      setProducto(data);
